...
not_cancelled<-filter(flights, !is.na(dep_delay) & !is.na(arr_delay))
not_cancelled
not_cancelled_year<-group_by(carrier, )
...
flights %>%
  ...
  group_by(origin, arr_delay, carrier) %>%
  count() %>%
  head(6)%>%
  ggplot(aes(x=reorder(origin), y=dest))+
  geom_bar(stat="identity")
...
# Which carrier experienced the most cancelled flights? flights %>%
flights %>%
